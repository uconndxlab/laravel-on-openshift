---
title: Quickstart
description: Deploy a Laravel application to OpenShift from scratch using the OpenShift Template
---

Deploy a Laravel application to OpenShift from scratch, including persistent storage for SQLite, health checks, and automated rollouts — using the [OpenShift Template](/templates/openshift-templates/) for a single-command deployment.

> For a step-by-step breakdown of each individual resource, see the [imperative quickstart reference](/quickstart-imperative/).
...
See the full [Containerfile reference](/images/containerfile/) for more options, including multi-stage builds for frontend assets and Nginx integration.
...
Now deploy all resources at once using the [OpenShift Template](/templates/openshift-templates/):
...
> **SQLite + replicas**: SQLite cannot handle concurrent writes from multiple pods. The template sets `replicas: 1`. See the [persistent storage guide](/guides/persistent-storage/#sqlite-and-replicas) for more.
...
- Learn how the [OpenShift Template](/templates/openshift-templates/) works and how to customize it
- Create a reusable [Helm chart](/templates/helm/) for deployments outside of OpenShift
- Set up [ArgoCD GitOps](/templates/argocd/) for automatic, declarative deployments
- Learn more about [building images with a Containerfile](/images/containerfile/) including multi-stage builds and Nginx integration
- Set up [automated deployments with Tekton](/ci-cd/automated-deploy/) so every `git push` triggers a build and rollout
- Read about [persistent storage options](/guides/persistent-storage/) including backup strategies and MySQL migration
- Explore [production patterns](/guides/production-patterns/) for queue workers, scheduled tasks, and blue-green deployments
