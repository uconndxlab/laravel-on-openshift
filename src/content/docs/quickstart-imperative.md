---
title: Quickstart (Imperative Reference)
description: Step-by-step imperative quickstart for deploying Laravel on OpenShift
---

> This page is the **original imperative quickstart** kept for reference. The recommended approach is the [template-based quickstart](/quickstart/), which achieves the same result with a single `oc process` command.
...
> **SQLite + replicas**: SQLite cannot handle concurrent writes from multiple pods. Keep replicas at 1: `oc scale deployment/myapp --replicas=1 -n <team>-dev`. See the [persistent storage guide](/guides/persistent-storage/#sqlite-and-replicas) for more.
