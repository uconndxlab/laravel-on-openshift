---
title: Automating Buildpack Builds on Git Push
description: Deprecated — Tekton pipeline with Paketo buildpacks (archived)
---

> **Archived**: This page covers the deprecated buildpack-based CI/CD approach. The current recommendation is the [Containerfile-based CI/CD guide](/ci-cd/automated-deploy/) which builds from a Dockerfile in your Git repo.
...
- **OpenShift pipeline ServiceAccount** — already set up with `quay-pull-<team>` (see [Quay getting-started](/quay/getting-started/#find-your-imagepullsecret))
...
Instead of `oc set image`, you can use the [OpenShift Template](/templates/openshift-templates/) to deploy the full set of resources (Deployment, Service, Route, PVCs, ConfigMap, Secret) in a single step:
...
With [ArgoCD](/templates/argocd/), your pipeline only needs to **build and push the image**. ArgoCD monitors your Git repository and syncs the deployment automatically — no deploy task required. The pipeline becomes:
...
See the [Deployment Templates overview](/templates/overview/) to choose the right approach for your team.
...
- **Mitigation**: run migrations as a standalone [Kubernetes Job](/guides/persistent-storage/#b-run-as-a-kubernetes-job) instead, or use a "blue-green" deployment pattern
...
Then add the `sqlite` PVC workspace to the pipeline and wire it through. See the [persistent storage guide](/guides/persistent-storage/#c-automate-in-cicd) for more details.
...
Paketo buildpacks accept environment variables to control the build. Set these in `project.toml` under `[build.env]` or as `env` entries in your Tekton task. For the complete Laravel-focused reference, see [the canonical Paketo environment variables table](/archive/images/paketo-buildpack/#environment-variables).
