...
This repo provides a Helm chart at `templates/helm/laravel/` that deploys the same Laravel configuration as the [OpenShift Template](openshift-templates.md) — Deployment, Service, Route, PVCs, ConfigMap, Secret, init container, probes, and resource limits — but with the flexibility of Helm's `values.yaml` and `--set` overrides.
...
The full chart source is at [`templates/helm/laravel/`](https://github.com/uconn/uconn-openshift-laravel/tree/main/templates/helm/laravel).
